...
def upsert_df(df: pd.DataFrame, table_name: str, primary_key: str, connection) -> bool:
...
token = boto3_client.generate_db_auth_token(DBHostname=DB_HOST, Port=PORT, DBUsername=DB_USER, Region=REGION)
...
if upsert_df(df_audio,"audio_features","id",connection) and upsert_df(df_tracks, "track_history", "played_at", connection):
    print("Success")
    connection.commit()
else:
    print("Failed")
